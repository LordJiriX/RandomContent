...
public class Randomcontent {
    ...
    public static final RegistryObject<CreativeModeTab> RANDOM_CONTENT_TAB = CREATIVE_MODE_TABS.register("RANDOM_CONTENT_TAB".toLowerCase(), () -> CreativeModeTab.builder().withTabsBefore(CreativeModeTabs.COMBAT).icon(() -> MULTI_TOOL.get().getDefaultInstance()).displayItems((parameters, output) -> {
        output.accept(MULTI_TOOL.get());
        RCData.allItemList.forEach(item -> {output.accept(item.get());});
        RCData.allBlockItemsList.forEach(item -> {output.accept(item.get());});
    }).build());
    ...
    private void commonSetup(final FMLCommonSetupEvent event) {
        LOGGER.info("----------------------------------------------------------------------------------------");
        LOGGER.warn("MODID: " + MODID + ":" + VERSION + " is in INDEV verion expect bugs and missing content!");
        LOGGER.info("----------------------------------------------------------------------------------------");
